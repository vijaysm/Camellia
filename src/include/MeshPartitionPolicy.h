...
//  static MeshPartitionPolicyPtr oneRankPartitionPolicy(int rank=0); // all cells belong to the rank specified
  ...
  static MeshPartitionPolicyPtr inducedPartitionPolicy(MeshPtr inducingMesh); // for two meshes that have the same cell indices, uses inducingMesh to define partitioning
  // ! Returns a partition policy that places the parent cell on the same processor as its first child (descendant).
  static MeshPartitionPolicyPtr inducedPartitionPolicyFromRefinedMesh(MeshTopologyViewPtr inducedMeshTopology, MeshPtr inducingRefinedMesh);
